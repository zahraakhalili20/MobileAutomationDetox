...
describe('Bidding Tests: Verifying Min bid amount', () => {
  let listingProduct = listingData.iphone15;
  let testUser = usersData.user_9,
    recommendedPriceAfterCommission,
    username;
  const inputStartPrice = '0',
    minAmountAllowed = 1;
  ...
  it('Setting Up Test Data', async () => {
    ...
    sellPriceWithcommission = minAmountAllowed - commissionValue - vatAmount;
    recommendedPriceAfterCommission =
      Math.floor((minAmountAllowed - commissionValue - vatAmount) * 100) / 100;
  });
  ...
  it('Verifying  Device Status - Questions 8', async () => {
    ...
    assert.equal(
      ...
      listingData.mobilesQuestions.Q4,
    );
  ...
  it('Verifying  Device Status - Questions 8', async () => {
    ...
    assert.equal(
      await DeviceStatusScreen.getTxtQuestion(question[0]),
  ...
  it('Verifying  Device Status - Battery Health', async () => {
    ...
    assert.equal(
      await DeviceStatusScreen.getQuestionInsideMenu(),
      listingData.mobilesQuestions.Q7,
    );
  ...
  it('Verifying Price Screen - Bidding', async () => {
    ...
    assert.equal(await DevicePriceBidScreen.getRecommendedPriceValue(), '');
    await DevicePriceBidScreen.enterStartPrice('-800' + '\n');
    assert.equal(await DevicePriceBidScreen.getRecommendedPriceValue(), '800');
    await DevicePriceBidScreen.enterStartPrice(minAmountAllowed.toString());
  ...
  it('Verifying  Price Confirmation', async () => {
    ...
    assert.equal(
      await GeneralComponentsSellNowScreen.getTxtPageHeader(),
      priceConfirmationTranslation.priceConfirmation,
    );
    assert.equal(
      await PriceConfirmationScreen.getTxtSectionHeader(),
      priceConfirmationTranslation.deviceStatus,
    );
...
*/
  ...
  it('Verifying SPP - Price', async () => {
    ...
    assert.equal(
      ...
      minAmountAllowed.toLocaleString(),
    );
    assert.equal(
  ...
  it('Verifying SPP - Ask Seller', async () => {
    assert.equal(
      ...
      singleProductTranslation.askSellerTitle,
    );
    assert.equal(
