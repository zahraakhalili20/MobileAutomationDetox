...
describe('My Wishlist: Verifying View Switching', () => {
    ...
    it('Filter home screen by mobiles and add  product to favorites from enlarged view', async () => {
        ...
        if (await SingleProductScreen.isBiddingProduct()) {
            productName1.price = (await SingleProductScreen.getTextStartingBidPriceValue()).toLocaleString()
            productName1.type = "bid product"
        }
        else
            productName1.price = (await SingleProductScreen.getTextProductSellingPriceBuyer()).toLocaleString()
    ...
    it('Filter by brand and add a product to favorites from enlarged view', async () => {
        ...
        await homeScreen.waitForScreenShown()
        await homeScreen.tapCategoryByName(exploreTranslation.accessories)
        await shopByCategoryScreen.waitForScreenShown()

        await shopByCategoryScreen.tapBrandByName(exploreTranslation.microsoft)
    ...
    it('Opening MyWishlist  default view is list view', async () => {
        ...
        assert.equal(await MyWishlistScreen.getNumberOfProductsShowing(), await MyWishlistScreen.getTextProductCount())
