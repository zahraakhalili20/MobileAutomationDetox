...
class ShopByCategory extends AppScreen {

    constructor() {
        const locator = by.id("screenCategory")
    ...
    get ProductCardBig(){
        const locator=by.id("bigcardSoumProduct")
        return element(locator)
    }
    get ProductCard(){
    ...
    async getTextShopByBrand() {
        return await commonFunction.getElementLabel(this.txtShopByBrand)
    }

    async getTextBrandName(index = 0) {
        return await commonFunction.getElementLabel(this.txtBrandName,index)
    }

    async tapBrandByName(name,RTL=true,direction=true) {
        const locator = element(by.id('txtBrandName').and(by.text(name)))
        if(RTL){
        if(direction)
            await commonFunction.waitForElementToVisibleWhileScrolling(locator, this.brandsScrollView, 'right',0,25)
        else 
        await commonFunction.waitForElementToVisibleWhileScrolling(locator, this.brandsScrollView, 'left',0,25)
        }
        else {
            if(direction)
            await commonFunction.waitForElementToVisibleWhileScrolling(locator, this.brandsScrollView, 'left',0,25)
            else await commonFunction.waitForElementToVisibleWhileScrolling(locator, this.brandsScrollView, 'right',0,25)
...
module.exports = new ShopByCategory()