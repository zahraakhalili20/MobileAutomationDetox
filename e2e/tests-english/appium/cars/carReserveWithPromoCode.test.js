...
describe('Reserve a car and enter a promo code', () => {
    ...
    it('setup testing data', async () => {
        ...
        sellPriceWithcommission = parseFloat(sell_price) - commission
         promo=`${String.fromCharCode(65 + Math.floor(Math.random() * 26))}${String.fromCharCode(65 + Math.floor(Math.random() * 26))}${Math.floor(10 + Math.random() * 90)}${Math.floor(10 + Math.random() * 90)}`;
        console.log(promo)
    ...
    it('Applying promo code Coupon', async () => {
    ...
    it('Asserting Summary screen UI', async () => {
        ...
        assert.equal(await reservationSummaryScreen.getProductPriceValue(), sell_price + " "+checkoutTranslation.riyal)
