...
describe('Testing Car SPP as a guest user', () => {
    let seller = usersData.user_42
    let newProductData = data.civic
    let listingProduct = listingData.civic
    let sellPriceWithcommission, product_id, product_id2, sell_price = "2500"
    ...
    it('Launching App and opening SPP', async () => {
        ...
        await MPPScreen.enterFilterPrice(mppTranslation.range2000To3000)
    ...
    it('Asserting  installments', async () => {
        ...
        assert.equal(await SingleProductScreen.getTextInstallementPrice(), installment.toString() + " SAR / Month")
    })
    ...
    it('Asserting Inspected and Guranteed by Soum section', async () => {
        ...
        assert.equal(await SingleProductScreen.getInspectionText(), singleProductTranslation.guaranteed)
        assert.equal(await SingleProductScreen.getInspectionSubText(), singleProductTranslation.yearOr20)
        ...
        assert.equal(await SingleProductScreen.getInspectionText(2), singleProductTranslation.transparency)
        assert.equal(await SingleProductScreen.getInspectionSubText(2), singleProductTranslation.allDefectsListed)
    ...
    it('Asserting Purchase process section', async () => {
        ...
        assert.equal(await SingleProductScreen.getPurchaseProcessStepNo(), singleProductTranslation.step1No)
        assert.equal(await SingleProductScreen.getPurchaseProcessStepText(), singleProductTranslation.reserveIt)

        assert.equal(await SingleProductScreen.getPurchaseProcessStepNo(1), singleProductTranslation.step2No)
        assert.equal(await SingleProductScreen.getPurchaseProcessStepText(1), singleProductTranslation.checkIt)

        assert.equal(await SingleProductScreen.getPurchaseProcessStepNo(2), singleProductTranslation.step3No)
        assert.equal(await SingleProductScreen.getPurchaseProcessStepText(2), singleProductTranslation.Pay)

        assert.equal(await SingleProductScreen.getPurchaseProcessStepNo(3), singleProductTranslation.step4No)
        assert.equal(await SingleProductScreen.getPurchaseProcessStepText(3), singleProductTranslation.transfer)
    ...
    it('asserting product Id showing for guest users', async () => {
