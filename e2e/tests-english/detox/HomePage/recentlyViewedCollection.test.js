...
describe('Testing recently viewed collection on home page', () => {
    ...
    it('list a product, open product, then go to home page and check recently viewed section now appearing with that product', async() => {
        ...
        assert.equal(await soumProductCard.checkForProductImage(model.nameEn),true)
        assert.equal(await soumProductCard.getTxtProductName(),model.nameEn)
    ...
    it('verify user can add product to favorites from recently viewed collection', async() => {
        await homeScreen.clickOnFavorites(collectionName,0,false)
        assert.equal(await homeScreen.isFavoriteSelected(collectionName,0,false),true)
        await bottomMenuScreen.tapMoreMenuTabIcon()
        await moreMenuScreen.waitForScreenShown()
    ...
    it('view multiple products, 5+ verify all, and verify user can scroll recently viewed collection and verify each product card(all expected attributes and elements to be shown for product card in this collection', async() => {
        ...
        for(i=0; i<6; i++) {
            ...
            assert.equal(await soumProductCard.checkForProductImage(model.nameEn,i),true)
            assert.equal(await soumProductCard.getTxtProductName(i),model.nameEn)
            isValid = sellPrices.includes(await soumProductCard.getTxtProductPrice(i));
            assert(isValid, "Product price doesn't match any sell_price value in listings");            
            assert.equal(await soumProductCard.getTxtProductCondition(model.nameEn,i),soumProductCardTranslation.excellent)
        }
    })
